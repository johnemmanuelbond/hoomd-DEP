...
class ExternalAnypole : public ExternalPotential
    {
    public:
    //! Constructor
    ExternalAnypole(std::shared_ptr<SystemDefinition> sysdef,
    ...
    virtual LongReal particleEnergyImplementation(uint64_t timestep,
        ...
        if (kr != 0.0 && m != 0.0){
            ...
            Scalar dtheta = atan2(major_axis.y, major_axis.x) - q0;
...
namespace detail
    {
void export_ExternalAnypole(pybind11::module& m)
    {
    pybind11::class_<ExternalAnypole,
                     ExternalPotential,
                     std::shared_ptr<ExternalAnypole>>(m, "ExternalAnypole")
        .def(pybind11::init<std::shared_ptr<SystemDefinition>,
                            float, std::shared_ptr<Variant>>())
        .def("setParams", &ExternalAnypole::setParamsPython)
        .def("getParams", &ExternalAnypole::getParamsPython)
        .def_property("electrode_gap",
                      &ExternalAnypole::getElectrodeGap,
                      &ExternalAnypole::setElectrodeGap)
        .def_property("electrode_orientation",
                      &ExternalAnypole::getElectrodeOrientation,
                      &ExternalAnypole::setElectrodeOrientation);
    }
